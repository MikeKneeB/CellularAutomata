...
from cellauto.core import GameWin
from cellauto.grids import life, ant, multi_ant

class TermWin(GameWin):

    def __init__(self):
        GameWin.__init__(self)
    ...
    def add_game(self, game_grid):
        super(TermWin, self).add_game(game_grid)
    ...
    def draw_welcome(self):
        try:
            ...
            self.stdscr.addstr(7,2, '4 - 34 Life')
            self.stdscr.addstr(8,2, 'h - Help')
            self.stdscr.addstr(9,2, 'q - quit')
            self.stdscr.refresh()
            while True:
                command = self.stdscr.getch()
                if command == 49:
                    self.add_game(life.LifeGrid(20, 20))
                    break
                elif command == 50:
                    self.add_game(ant.AntGrid(20, 20))
                    break
                elif command == 51:
                    self.add_game(multi_ant.MultiAntGrid(20,20))
                    break
                elif command == 52:
                    self.add_game(life.LifeGrid(20, 20, [3,4], [3,4]))
                    break
                elif command == 104:
                    pass
                elif command == 113:
